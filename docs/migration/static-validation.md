...
It runs the three focused validators in a fixed order and then checks the
package contract directly:
...
1. JSON manifest and marketplace shape, shared Plugin identity, and the
   ...
   changelog;
2. the exact 23-file bundled Canon inventory, no nested Canon root, exactly
   eight Skill directories, and every `SKILL.md` entry point;
